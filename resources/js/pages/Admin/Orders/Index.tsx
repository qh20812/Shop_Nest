...
interface PageProps {
  ...
  paymentStatusOptions: Record<number, string>;
  currency?: string;
  exchangeRates?: Record<string, number>;
}

export default function Index({ orders, filters, statusOptions, paymentStatusOptions, currency = 'VND', exchangeRates = {} }: PageProps) {
  ...
  const getOrderActions = (order: Order) => [
    ...
    },
    {
      ...
      disabled: order.status >= 2 // Disabled if shipped or delivered
    },
    {
      label: 'Update Status',
      icon: 'bx-edit',
    ...
    },
    {
      ...
      disabled: order.payment_status !== 1 // 1 = paid
    },
    {
      label: 'Print Invoice',
      icon: 'bx-printer',
    ...
    <AppLayout>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <h4 style={{ margin: 0, color: 'var(--dark)', fontSize: '18px' }}>
              {orders.data.filter(order => order.status === 0).length}
            </h4>
            <p style={{ margin: 0, color: 'var(--dark-grey)', fontSize: '14px' }}>
              {t('Pending Orders')}
            </p>
        ...
        <div style={{
          ...
          <div>
            <h4 style={{ margin: 0, color: 'var(--dark)', fontSize: '18px' }}>
              {orders.data.filter(order => order.status === 3).length}
            </h4>
            <p style={{ margin: 0, color: 'var(--dark-grey)', fontSize: '14px' }}>
              {t('Completed Orders')}
            </p>
        ...
        <div style={{
          ...
          <div>
            <h4 style={{ margin: 0, color: 'var(--dark)', fontSize: '18px' }}>
              {orders.data.filter(order => order.status === 4).length}
            </h4>
            <p style={{ margin: 0, color: 'var(--dark-grey)', fontSize: '14px' }}>
              {t('Cancelled Orders')}
            </p>
