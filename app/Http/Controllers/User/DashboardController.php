...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $orderStats = [
            ...
            'delivered_orders'  => $user->orders()->where('status', 3)->count(),
            'cancelled_orders'  => $user->orders()->where('status', 4)->count(),
            'total_spent'       => $user->orders()->where('status', 3)->sum('total_price'),
        ];
